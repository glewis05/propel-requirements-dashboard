...
import type { TestCaseStatus, TestStep, Database } from "@/types/database"
...
export async function createTestCase(data: TestCaseFormData) {
  ...
  const insertData: TestCaseInsert = {
...
export async function archiveTestCase(testCaseId: string) {
  ...
  const archiveUpdate: TestCaseUpdate = { is_archived: true, status: "deprecated" }
  const { error } = await supabase
    .from("test_cases")
...
export async function reviewTestCase(testCaseId: string) {
  ...
  const reviewUpdate: TestCaseUpdate = {
    human_reviewed: true,
