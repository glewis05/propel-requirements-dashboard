...
export default async function ValidationPage() {
  ...
  // Fetch stories in UAT with test progress (exclude soft-deleted)
  interface StoryRow { story_id: string; title: string }
  const { data: uatStories } = await supabase
    .from("user_stories")
    .select("story_id, title")
    .eq("status", "In UAT")
    .is("deleted_at", null) as { data: StoryRow[] | null; error: Error | null }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Test validation overview and management
        </p>
      </div>
