...
import type { DistributionMethod, ExecutionStatus, AssignmentType } from "@/types/database"
...
export async function executeAssignment(config: AssignmentConfig): Promise<{
  ...
  if (cvGroups.size > 0) {
    const cvGroupInserts = Array.from(cvGroups.keys()).map(testCaseId => ({
      cycle_id: config.cycleId,
    ...
    const { data: createdCvGroups, error: cvError } = await supabase
      ...
      .insert(cvGroupInserts as never[])
  ...
  const assignments = (createdExecutions || []).map(exec => {
    const isCv = cvGroups.has(exec.test_case_id)
    return {
      cycle_id: config.cycleId,
      execution_id: exec.execution_id,
      assignment_type: (isCv ? "cross_validation" : "primary") as AssignmentType,
