-- ============================================================================
-- Migration 007: UAT Tester Portal Schema
-- Adds UAT cycles, tester management, test patients, and cross-validation
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS uat_cycles (
  ...
  -- equal (round-robin) or weighted (by capacity)
  cross_validation_enabled BOOLEAN NOT NULL DEFAULT false,
  cross_validation_percentage INTEGER DEFAULT 20,
  -- Percentage of tests to cross-validate (e.g., 20 = 20%)
  validators_per_test INTEGER DEFAULT 3,
  -- Number of testers to assign for cross-validation tests
  start_date DATE,
  end_date DATE,
...
CREATE INDEX IF NOT EXISTS idx_uat_cycles_program_id ON uat_cycles(program_id);
CREATE INDEX IF NOT EXISTS idx_uat_cycles_status ON uat_cycles(status);
CREATE INDEX IF NOT EXISTS idx_uat_cycles_created_by ON uat_cycles(created_by);
...
CREATE TABLE IF NOT EXISTS cycle_testers (
  ...
  -- 100 = full capacity, 50 = half capacity, etc.
...
CREATE INDEX IF NOT EXISTS idx_cycle_testers_cycle_id ON cycle_testers(cycle_id);
CREATE INDEX IF NOT EXISTS idx_cycle_testers_user_id ON cycle_testers(user_id);
CREATE INDEX IF NOT EXISTS idx_cycle_testers_is_active ON cycle_testers(is_active);
...
CREATE TABLE IF NOT EXISTS test_patients (
  ...
  date_of_birth DATE,
  description TEXT,
  -- Describes the test scenario this patient represents
  test_data_notes TEXT,
  -- Notes about what test data exists for this patient
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  created_by TEXT NOT NULL REFERENCES users(user_id),
...
CREATE INDEX IF NOT EXISTS idx_test_patients_program_id ON test_patients(program_id);
CREATE INDEX IF NOT EXISTS idx_test_patients_is_active ON test_patients(is_active);
CREATE INDEX IF NOT EXISTS idx_test_patients_mrn ON test_patients(mrn);
...
CREATE INDEX IF NOT EXISTS idx_tester_acknowledgments_cycle_id ON tester_acknowledgments(cycle_id);
CREATE INDEX IF NOT EXISTS idx_tester_acknowledgments_user_id ON tester_acknowledgments(user_id);

-- ============================================================================
-- 5. CYCLE ASSIGNMENTS TABLE
...
CREATE TABLE IF NOT EXISTS cycle_assignments (
  ...
  -- primary or cross_validation
  cross_validation_group_id UUID,
  -- Links to cross_validation_groups if cross-validation
...
CREATE INDEX IF NOT EXISTS idx_cycle_assignments_cycle_id ON cycle_assignments(cycle_id);
CREATE INDEX IF NOT EXISTS idx_cycle_assignments_execution_id ON cycle_assignments(execution_id);
CREATE INDEX IF NOT EXISTS idx_cycle_assignments_type ON cycle_assignments(assignment_type);
CREATE INDEX IF NOT EXISTS idx_cycle_assignments_cv_group ON cycle_assignments(cross_validation_group_id);

-- ============================================================================
-- 6. CROSS VALIDATION GROUPS TABLE
-- Groups test cases for cross-validation comparison
-- ============================================================================
CREATE TABLE IF NOT EXISTS cross_validation_groups (
...
-- Add foreign key to cycle_assignments after cross_validation_groups exists
ALTER TABLE cycle_assignments
ADD CONSTRAINT fk_cycle_assignments_cv_group
FOREIGN KEY (cross_validation_group_id)
REFERENCES cross_validation_groups(group_id) ON DELETE SET NULL;

-- Indexes for cross_validation_groups
CREATE INDEX IF NOT EXISTS idx_cross_validation_groups_cycle_id ON cross_validation_groups(cycle_id);
CREATE INDEX IF NOT EXISTS idx_cross_validation_groups_test_case_id ON cross_validation_groups(test_case_id);
...
ADD COLUMN IF NOT EXISTS cycle_id UUID REFERENCES uat_cycles(cycle_id) ON DELETE SET NULL;

ALTER TABLE test_executions
ADD COLUMN IF NOT EXISTS test_patient_id UUID REFERENCES test_patients(patient_id) ON DELETE SET NULL;

-- Index for new columns
CREATE INDEX IF NOT EXISTS idx_test_executions_cycle_id ON test_executions(cycle_id);
CREATE INDEX IF NOT EXISTS idx_test_executions_test_patient_id ON test_executions(test_patient_id);
...
BEGIN
  ...
  SELECT
    ...
    -- Agreement = all completed and all have same pass/fail status
    (
